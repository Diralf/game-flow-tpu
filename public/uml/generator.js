class UmlGenerator {
    ...
    async generate(t) {
        ...
        const idMap = new Map(fullCards.map(card => [
            card.id,
            'card' + card.idShort
        ]));
        const cardsMap = new Map(fullCards.map(card => [
            card.id,
            card
        ]));
        ...
        for (const card of fullCards) {
            if (card.shared.relates) {
                ...
                if (cardName) {
                    const linksUml = card.shared[window.depends.childesFieldName].map(link => {
                        const linkName = idMap.get(link);
                        return linkName ? `${this.name(cardName)} --> ${this.name(idMap.get(link))}` : '';
                    });
        ...
        const coloredTypes = Array.from(cardTypesMap).map(([key, options]) => `FontColor<<${key}>> ${options.umlColor}`).join('\n');
    ...
    registerChildes(card, idMap, cardsMap, ident = 0) {
        ...
        stateUml = identText + `state "${(card.shared.cardType || 'task').toUpperCase()}\\n ${url}" as ${cardShortName} <<${card.status}>> <<${card.shared.cardType}>> <<${'p' + card.shared.severity}>> {\n `;

        const childes = card.shared[window.links.childesFieldName];
        if (childes) {
            for (const childId of childes) {
                stateUml += this.registerChildes(cardsMap.get(childId), idMap, cardsMap, ident + 2)
            }
        }
        const severity = window.severity.typesMap.get(card.shared.severity) || window.severity.typesMap.get(window.severity.default);

        stateUml += identText + '}\n';
        stateUml += identText + `${cardShortName} : ${severity.name}\n`;
        stateUml += identText + `${cardShortName} : ${card.status}\n`;
