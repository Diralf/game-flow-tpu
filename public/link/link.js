class Links {
    ...
    async addBothLink(t, toCard, options) {
        await this.addLink(t, options.currentCard, toCard, options.fieldName, options.reverseFieldName, options.single);
        await this.addLink(t, toCard, options.currentCard, options.reverseFieldName, options.fieldName, options.reverseSingle);
    }
    ...
    async removeBothLink(t, toId, options) {
        ...
        await this.removeLink(t, toId, options.currentCardId, options.reverseFieldName);

        await this.removeSingle(t, options.currentCardId, options.fieldName, options.single);
        await this.removeSingle(t, toId, options.reverseFieldName, options.reverseSingle);
    }

    async removeLink(t, fromId, toId, fieldName) {
        const links = await t.get(fromId, 'shared', fieldName, []);
        await t.set(fromId, 'shared', fieldName, links.filter(link => link !== toId));
    }
    ...
    async openStartMenu(t, options) {
        ...
        const availableTypes = options.direction ? (await window.cardType.getTypeRelation(t, type, options.direction)) : [type];
        const isAddEnabled = (!options.single || options.linkCount === 0) && availableTypes.length > 0;
        ...
        if (isCreateEnabled) {
            items.push({
                text: ' > Create New',
                callback: (t, opts) => { this.createLink(t, options) }
            });
        }
        if (isAddEnabled) {
            items.push({
        ...
        const links = await t.get(options.currentCardId, 'shared', options.fieldName, []);
        const linkedCards = cards.filter(card => links.includes(card.id));

        items.push(...linkedCards.map(card => ({
            text: card.name,
    ...
    async openFullList(t, options) {
        const cards = await t.cards('id', 'name', 'idShort');
        const links = await t.get(options.currentCardId, 'shared', options.fieldName, []);
        const unlinkedCards = cards.filter(card => !links.includes(card.id)).filter(card => card.id !== options.currentCardId);
        const availableCards = await this.filterList(t, unlinkedCards, options.currentCardId, options.direction);
        return t.popup({
            title: 'Add Link',
            search: {
    ...
    async defineCardDetailBadge(t, options) {
        ...
        if (actualLinks.length !== data.length || data.some((link, index) => !actualLinks.includes(link))) {
        ...
        if (options.single) {
            const linkedCard = await t.get("card", "shared", options.fieldName + 'Card');
    ...
    async filterList(t, cards, currentCardId, direction) {
        const typeCards = await Promise.all(cards.map(card => t.get(card.id, 'shared', cardType.fieldName)));
        const currentCardType = await t.get(currentCardId, 'shared', cardType.fieldName);

        const typeRelations = await cardType.getTypesRelations(t);
        const currentCardLevelIndex = typeRelations.findIndex(level => level.includes(currentCardType));
        const availableTypes = typeRelations[this.selectLevel(currentCardLevelIndex, direction)];
        if (!availableTypes) {
            return [];
        }
